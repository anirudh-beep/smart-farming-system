...
class WeatherService {
  ...
  async getWeatherForecast(location) {
    try {
      console.log('Getting weather forecast for location:', location);
  ...
  getMockWeatherData(location) {
    ...
    const mockResult = {
      ...
      forecast: Array.from({ length: 7 }, (_, i) => {
        ...
        const temp = 25 + Math.floor(Math.random() * 10); // 25-35°C
        const rain = Math.floor(Math.random() * 15 * 10) / 10; // 0-15mm with 1 decimal
        ...
        return {
          ...
          condition: rain > 5 ? 'Light rain' : 'Partly cloudy',
    ...
    console.log('Mock weather data generated:', mockResult);
    return mockResult;
  }
